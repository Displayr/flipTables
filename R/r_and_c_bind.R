...
#' Takes a sequence of vectors or tables and merges them, similar to
#' rbind and cbind, except that it matches based on names. If there
#' are no names, matching is based upon indices.
#' @param ... The tables or vectors to be merged.
#' @param keep.all If \code{"TRUE"}, even non-matching rows/columns
#'     are retained. Ignored and treated as \code{"TRUE"} if matching
#'     is based upon indices.
#' @noRd
cbindWithLabels <- function(..., keep.all)
{
    # rbind does not work on matrices with different ncols. However we use rbind to extract rownames.
    # So temporarily set all ncols to 1 for matrices. Vica versa for cbind.
    tables <- list(...)
    ...
    named.objects <- sapply(object.names, exists)
    names(tables)[named.objects] <- object.names[named.objects]
    bind.tables <- suppressWarnings(do.call(cbind, tables))
    ...
    if(is.list(tables[[1]]) && !is.data.frame(tables[[1]]))
        return(cbindWithLabels(tables[[1]], keep.all = keep.all))

    merged <- MergeTables(tables, direction = "Side-by-side",
                          nonmatching = if (keep.all) "Keep all" else "Matching only")
...
{
    tbls <- lapply(list(...), function(x) if (length(dim(x)) == 2L) t(x) else x)
    return(t(do.call(cbindWithLabels, c(tbls, keep.all = keep.all))))
}

RbindOld <- function(..., rows = TRUE, keep.all = TRUE)
    return(rbindAndCbindWithLabels(..., rows = TRUE, keep.all = keep.all))
...
{
        return(cbindWithLabels(..., keep.all = keep.all))
}
