...
test_that("Quarters", {
    ...
    expect_error(StackYears(tail(vals, 6), tail(dates, 6)), NA)
    # First quarter of 2016 excluded
    expect_error(StackYears(tail(vals, 5), tail(dates, 5)),
                 "At least one whole year of data is required.")

    # Calendar == FALSE, last 4 quarters, no error
    expect_error(StackYears(tail(vals, 4), tail(dates, 4), calendar = FALSE), NA)
    # Calendar == FALSE, last 3 quarters
    expect_error(StackYears(tail(vals, 3), tail(dates, 3), calendar = FALSE),
                 "At least one whole year of data is required.")
})
...
test_that("Months", {
    ...
    expect_error(StackYears(tail(dat, 21)), NA)
    # Jan 2019 excluded
    expect_error(StackYears(tail(dat, 20)),
                 "At least one whole year of data is required.")

    # Calendar == FALSE, last 12 months, no error
    expect_error(StackYears(tail(dat, 12), calendar = FALSE), NA)
    # Calendar == FALSE, last 11 months
    expect_error(StackYears(tail(dat, 11), calendar = FALSE),
                 "At least one whole year of data is required.")
})

test_that("Insufficient data with weekly data", {
    # All of 2016 included, no error
    suppressWarnings(expect_error(StackYears(tail(adhd, 83), tail(dates, 83)), NA))
    # First week of 2016 excluded
    expect_error(StackYears(tail(adhd, 82), tail(dates, 82)),
                 "At least one whole year of data is required.")
})
